...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Ноя 06 2022 г., 00:51
...
CREATE TABLE `bank_card-product` (
  ...
  `id_card-images` int DEFAULT NULL,
...
INSERT INTO `bank_card-product` (`id_product`, `name`, `description`, `preview_image`, `id_card-images`, `id_category`, `id_type_card`) VALUES
(1, 'Citron Card', 'Зарабатывайте деньги, а не бонусы. Закажите дебетовую карту с платежной системой «Мир» и получайте кэшбэк в любимых категориях покупок', 'img\\cards\\Preview-Cards\\Citron-Card.png', NULL, 3, 1),
(2, 'Citron Game', 'Дебетовая карта геймера. Закажите карту со своим никнеймом, копите бонусы, получайте игры и технику бесплатно', 'img\\cards\\Preview-Cards\\Citron-Game.png', NULL, 2, 1);
...
(12, 'Павел', 'Сержантов', 'Андреевич', 'newdesign@mail.ru', 'fcea920f7412b5da7be0cf42b8c93759', NULL, '+79001000001', 'nussll', 'user', 'default');
...
ALTER TABLE `bank_card-product`
  ...
  ADD KEY `id_card-images` (`id_card-images`),
  ADD KEY `id_category` (`id_category`),
  ADD KEY `id_type_card` (`id_type_card`);
...
--
ALTER TABLE `bank_card-category`
  MODIFY `id_card-category` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
--
ALTER TABLE `bank_card_image-path`
  MODIFY `id_card_image-path` int NOT NULL AUTO_INCREMENT;
...
ALTER TABLE `bank_card-product`
  ...
  ADD CONSTRAINT `bank_card-product_ibfk_2` FOREIGN KEY (`id_card-images`) REFERENCES `bank_card_image-path` (`id_card_image-path`);
