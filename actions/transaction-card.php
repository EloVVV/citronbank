    ...
    if(isset($_POST)){
        ...
        || empty($transaction_sum)) {
            ...
            if(isset($_POST['payment'])) {
                header('Location: ../?page=transaction_for_payment');
...
// Конвертированный
            ...
            if($fromCurrency === 'RUB') {
                $procent = ($transaction_sum * 5) / 100;
                $send_sum = $transaction_sum + $procent;
                $nowCurrency = $fromCurrencyID;
                ...
                $transaction_sum = $transaction_sum * get_currency(strval($fromCurrency), 1)[0]->Value;
                
            }

            $fromBalanceNew = $fromUser['balance'] - $send_sum;

            // Можно сплитом забрать символы
        ...
        if(count($_SESSION['errors']) === 0) {
            ...
            //     $toBalanceNew = round($toBalanceNew, 4);
            //     $toBalanceUpdated = round($toBalanceUpdated, 4);
            // }
            // -----
            ...
            $fromBalanceNew = round($fromBalanceNew, 4);
            $fromBalanceUpdated = round($fromBalanceUpdated, 4);
            ...
            if($status === 'Отправлено') {
                $time = time();
                if(isset($send_sum)) {
                    $transaction_sum_from = $send_sum;

                    $capital_amount = $database->query("SELECT * FROM `bank_capital` WHERE `currency`='$nowCurrency'")->fetch(PDO::FETCH_ASSOC);

                    $updatedAmount = $capital_amount['amount'] + $procent;
                    $database->query("UPDATE `bank_capital` SET `amount`='$updatedAmount' WHERE `currency`='$nowCurrency'")->fetch(PDO::FETCH_ASSOC);
                } else {
                ...
                $insertTransaction = $database->prepare("INSERT INTO `bank_transactions`
                 (`from_id`, `to_id`, `summary_from`, `summary_to`, `commission`, `status`, `organizationID`,`created_at`, `updated_at`) 
                VALUES (:from_id, :to_id, :summary_from, :summary_to, :commission, :status, :organizationID,:created_at, :updated_at)");
